...
execute if score @s cmsb_pp.task.id matches 12 run title @s actionbar [{"text": "クリーパーの爆破回数: "},{"score": {"name": "@s","objective": "cmsb_pp.task.progress"},"color": "gold"},{"text": " /1","color": "red"}]
...
execute if score @s cmsb_pp.task.id matches 15 run title @s actionbar [{"text": "オブザーバーを被りプレイヤーを観察する: "},{"score": {"name": "@s","objective": "cmsb_pp.task.progress"},"color": "gold"},{"text": " /1","color": "red"}]